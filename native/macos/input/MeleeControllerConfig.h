...
// Mapping adapted from McDandle/melee-macos-recomp. See README.md for provenance.
#pragma once
...
// handles other controller types and supplies one common mapping to the game.
...
// Dolphin's SDLGamepad backend, which maps face buttons by their positions.
inline void ConfigureController(Common::IniFile::Section *section, const std::string &device,
                                int port)
{
  ...
  for (const auto &key : keys)
  {
    ...
    {
      expression += (expression.empty() ? "" : " | ") + std::string("`Quartz/0/Keyboard & Mouse:") +
                    key[1] + "`";
    }
  ...
  // Xbox triggers have no separate click. Keep their full analog travel and
  // turn the last 10 percent into the GameCube trigger's digital press.
